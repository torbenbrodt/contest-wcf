	...
	public function execute($tagArgs, Template $tplObj) {
		...
		if ($tagArgs['pages'] > 1) {
			...
			if ($tagArgs['pages'] > self::SHOW_LINKS) {
				// jumper
				$html .= '<li><a onclick="var result = prompt(\''.WCF::getLanguage()->get('wcf.global.page.input').'\', \''.$tagArgs['pages'].'\'); if (typeof(result) != \'object\' && typeof(result) != \'undefined\') document.location.href = (\''.StringUtil::replace("'", "\'", $link).'\').replace(/%d/, result);">&hellip;</a></li>'."\n";
