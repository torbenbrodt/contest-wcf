...
class LanguageEditor extends Language {
	...
	public static function search($search, $replace = null, $languageID = null, $useRegex = 0, $caseSensitive = 0, $searchVariableName = 0) {
		...
		if ($useRegex) {
			...
			$searchCondition .= "LIKE '%".escapeString($search)."%'";
		}
		...
		// search
		$updatedItems = array();
		$sql = "SELECT		languageItemID, languageItem, languageID, languageCategoryID, languageItemValue
			FROM		wcf".WCF_N."_language_item
		...
		while ($row = WCF::getDB()->fetchArray($result)) {
			if ($replace !== null) {
				...
				$matches = 0;
				if ($useRegex) {
					$newValue = preg_replace('/'.$search.'/s'.(!$caseSensitive ? 'i' : ''), $replace, $row['languageItemValue'], -1, $matches);
				}
				else {
					if ($caseSensitive) $newValue = StringUtil::replace($search, $replace, $row['languageItemValue'], $matches);
					else $newValue = StringUtil::replaceIgnoreCase($search, $replace, $row['languageItemValue'], $matches);
				}
